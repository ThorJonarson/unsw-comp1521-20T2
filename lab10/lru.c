...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    int physical_to_use = 0;
    for (int i = 0; i < n_physical_pages, i++) {
        if (ipt[i].virtual_page == virtual_page) {
            physical_to_use = i;
            break;
        }
        if (ipt[i].last_access_time < ipt[physical_to_use].last_access_time)
            physical_to_use = i;
    }
    ...
    ipt[physical_to_use].last_access_time = access_time;
    ipt[physical_to_use].virtual_page = virtual_page;
}
